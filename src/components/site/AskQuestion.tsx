...
export function AskQuestion() {
  ...
  return (
    <div className="glass rounded-3xl p-6 sm:p-8 shadow-elegant border-warm">
      <p className="text-xs uppercase tracking-widest text-accent font-semibold">Ask a Question · Confidential</p>
      <h3 className="text-2xl font-bold mt-1 mb-2">A private answer from our medical team</h3>
      <p className="text-sm text-muted-foreground mb-5">Patient-doctor trust is the foundation of every healing relationship we build. Your message is handled with full discretion.</p>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2 p-1 rounded-xl bg-secondary">
            <button type="button" onClick={() => setChannel("phone")} className={`px-4 py-2 rounded-lg text-sm font-medium inline-flex items-center gap-1.5 transition-all ${channel === "phone" ? "bg-background shadow-card text-primary" : "text-muted-foreground"}`}>
              <Phone className="size-4" /> Phone
            </button>
            <button type="button" onClick={() => setChannel("email")} className={`px-4 py-2 rounded-lg text-sm font-medium inline-flex items-center gap-1.5 transition-all ${channel === "email" ? "bg-background shadow-card text-primary" : "text-muted-foreground"}`}>
